...
class FragmentInfoUser : Fragment() {
    ...
    private fun cargarHistorial() {
        ...
        //Si hay rutinas
        val listaHistorial = historialDb.getSesiones(userDb.getUsuarioSeleccionado()?.id!!)//Obtienemos la lista de rutinas
        val adapter= AdapterHistorial(requireContext(), listaHistorial) //Creamos el adapter
        val layoutRutina= LayoutInflater.from(requireContext()).inflate(R.layout.fragment_listas,null)//inflamos la rutina
        layoutRutina.background= ContextCompat.getDrawable(requireContext(),R.drawable.background)//Le cambiamos el fondo
        val lvSesiones = layoutRutina.findViewById<ListView>(R.id.listas)//Rlacionamos con nuestro layout
    ...
    private fun enviarIdSesion(lvSesiones:List<Sesion>,position:Int) {
            ...
            requireActivity().supportFragmentManager.beginTransaction()
                .replace(R.id.contenedor_fragments, fragmentHistorial)
                .addToBackStack(null)
                .commit()
    }
